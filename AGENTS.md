...
- 自定义主题文件只能改配色（colors）、符号（glyphs）
  ...
  随附主题统一纯黑背景（#000000）
- 配色按明暗分级：faint（装饰线/阴影/远端歌词等最弱层）< muted（次要文本）< foreground（正文）；
  双强调色小剂量使用：accent 承载选中/播放/进度填充等主高亮，accent2 点缀品牌标记、
  进度热端、弹窗标题与按键名；error 仅用于错误
- 视觉语言为纯字符栅格：头部品牌行 + 整宽细分隔线（glyphs.rule）、列表为居中限宽
  （≤72 列）的等宽列网格（光标/序号/标记/主文本列/次文本列/右列，列宽取可见项最大值，
...
- 歌词区固定在正文与状态栏之间，全局常驻；当前句居中高亮，上下各半，开头结尾留空；
  其余句按与当前句的距离从前景色逐行渐变到微光色（渐变表由主题色预计算 16 档，
  按距离取档，随歌词行数自适应；主题色无法解析时回退前景/微光两档）；
...
- 已播放/空余部分为主题符号（progress_fill/progress_empty），颜色取 accent/faint；
  已播放部分最后一格为"热端"：progress_head 非空时用该符号、否则沿用填充符，颜色取 accent2
- 刷新为 1Hz 按需定时（`scheduleProgressTick`），仅播放中运行；暂停、待播、无播放时
  完全停止；定时器带序号（progressSeq）丢弃暂停/切歌后的残留定时器

整体布局（自上而下，代码固定）：头部（左品牌标记 `▞ 木末 molpe`，右模式·音质·音量微光信息）→
整宽分隔线 → 正文（歌曲页带 `▍ 歌单名 + 曲目数` 标题行）→ 歌词区 → 进度条 →
状态栏（播放图标 + 曲目 + 位置 + 待播数，次要信息已移至头部右侧）→ 帮助行（自绘，按键 accent2 +
说明 faint，宽度不足时从尾部舍弃条目；不再使用 bubbles/help）。
栅格锚定：正文区固定高度（总高 - chromeFixed(6) - 歌词行数 - 各区块间距），超出截断、不足补空行，
底部区块（歌词/进度/状态/帮助）恒定锚定在屏幕底部，不随正文内容多少浮动；
登录页二维码只补不截（截断将无法扫描）；加载/错误提示在正文区水平居中。
...
- 配置项（`config.json`）：`quality` 音质、`auto_play` 启动自动开播（默认 false，恢复为待播）、
  ...
  `lyric_gap_above`/`lyric_gap_below` 歌词区与上方正文、下方进度条之间的空行数（默认均为 1，范围 0-10，0 即紧凑）、
  `playback_gap_below` 播放信息块（进度条 + 当前歌曲信息）与操作帮助行之间的空行数（默认 1，范围 0-10）、
  `random_no_repeat` 随机播放回避窗口（默认 1，范围 0-100；0 为完全随机，超过歌单长度自动收敛，修改后重启生效）、
  `song_gap` 相邻两首歌之间的静默间隔秒数（默认 0 连播，范围 0-600；间隔中下一首呈待播暂停状态，
  到期自动开播，播放键跳过等待立即开播，切歌/点歌取消间隔直接开播目标曲，修改后重启生效）、
...
- 队列核心在 `internal/queue`（纯逻辑、可单测，不依赖网络与外部进程）：
  - 顺序/列表循环/随机三种模式；随机在歌单内等概率选取，但不与最近
    `random_no_repeat` 首已播放且属于当前歌单的歌曲重复（按歌曲 ID 去重，
    窗口自动收敛为 min(n, 歌单长度-1)，单曲歌单重复播放；0 为完全随机）
...
- 本地打包必须保持仓库干净：`mkdir -p /tmp/molpe-build/build /tmp/molpe-build/srcdest &&
  BUILDDIR=/tmp/molpe-build/build SRCDEST=/tmp/molpe-build/srcdest PKGDEST=dist makepkg -f`
  （BUILDDIR 与 SRCDEST 必须是不同子目录，相同路径会导致源码缓存与构建目录冲突），
