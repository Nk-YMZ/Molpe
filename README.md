...
- **轻量低功耗**：前后端分离——播放、队列、MPRIS 都在 `molpe daemon` 守护进程里，TUI 只是个显示终端，`Q` 一按界面消失、音乐照放，再敲 `molpe` 界面回来。守护进程稳态零周期定时器（歌词在下一句到来前才醒一次，进度条暂停即停，位置全靠锚点外推），耗的电基本就是 mpv 解码和网络。给超轻本的续航添砖加瓦（微小
